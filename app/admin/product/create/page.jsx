...
export default function Page() {
  ...
  // ← NEW : store PDF previews + file objects
  const [pdfPreviews, setPdfPreviews] = useState([]); // [{name, file}, ...]
  ...
  const onSubmit = (data) => {
    console.log("Form Data:", {
      ...
      coverImage: data.coverImage?.[0]?.name || "No cover image",
      images: imagePreviews.map((p) => p.file.name),
      documents: pdfPreviews.map((p) => p.file.name), // ← NEW
      links: data.links,
    });
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex items-center gap-4 flex-1">
              ...
              <select
                {...register("productCategory", { required: "请选择作品分类" })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
              >
              ...
              {errors.productCategory && (
                <span className="text-red-500 text-sm">{errors.productCategory.message}</span>
              )}
            </div>
          </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <textarea
              {...register("productDescription", { required: "请输入作品描述" })}
              placeholder="请输入作品描述"
              rows={4}
              className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none resize-none"
            />
            {errors.productDescription && (
              <span className="text-red-500 text-sm">{errors.productDescription.message}</span>
            )}
          ...
          {/* 上传封面 */}
          <div>
            <label className="text-sm font-medium text-gray-700 mb-2 block">上传封面</label>
              ...
              <label htmlFor="cover-upload" className="cursor-pointer">
                {coverPreview ? (
                  ...
                  <>
                    ...
                    <span className="text-sm text-gray-600">上传产品封面</span>
                  </>
                )}
              </label>
            </div>
          ...
          <div>
            <div className="flex items-center gap-4 mb-2">
              ...
              <span className="text-sm text-[#636363]">单个文件最大支持20M，支持jpg/png</span>
            </div>
          ...
          <div>
            <div className="flex items-center gap-4 mb-2">
              <label className="text-sm font-medium text-gray-700">上传文档</label>
            ...
            {pdfPreviews.length > 0 && (
              <div className="flex flex-wrap gap-3 mt-4">
                {pdfPreviews.map((p, idx) => (
                  <div
                    key={idx}
                    className="relative inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-50 text-green-700 text-sm"
                  >
                    {p.name}
                    <button
                      type="button"
          ...
          {/* 上传链接 */}
          <div>
            <label className="text-sm font-medium text-gray-700 mb-2 block">上传链接</label>
          ...
          {/* 提交按钮（如果需要） */}
          {/* <button type="submit" className="mt-6 px-6 py-2 bg-blue-600 text-white rounded-lg">上传</button> */}
        </form>
      </div>
    </div>
  );
